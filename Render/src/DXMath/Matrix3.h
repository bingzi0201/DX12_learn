...
namespace Math {
	...
	public:
		...
		INLINE XM_CALLCONV Matrix3(const Vector3& x, const Vector3& y, const Vector3& z) {
			m_mat[0] = x;
			m_mat[1] = y;
			m_mat[2] = z;
		}
		INLINE XM_CALLCONV Matrix3(const Matrix3& m) {
			m_mat[0] = m.m_mat[0];
			m_mat[1] = m.m_mat[1];
			m_mat[2] = m.m_mat[2];
		}
		INLINE XM_CALLCONV Matrix3(Quaternion q) { *this = Matrix3(XMMatrixRotationQuaternion(q)); }
		INLINE explicit XM_CALLCONV Matrix3(const XMMATRIX& m) {
			m_mat[0] = Vector3(m.r[0]);
			m_mat[1] = Vector3(m.r[1]);
			m_mat[2] = Vector3(m.r[2]);
		}
		INLINE explicit XM_CALLCONV Matrix3(EIdentityTag) {
